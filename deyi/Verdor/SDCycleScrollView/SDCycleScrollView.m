...
- (void)setupMainView
{
    ...
    UIView *bview = [[UIView alloc] initWithFrame:CGRectMake(0, self.bounds.size.height-25, self.bounds.size.width, 25)];
    bview.backgroundColor = [[UIColor blackColor] colorWithAlphaComponent:0.5];
    ...
    [_tagLabel setContentHuggingPriority:UILayoutPriorityRequired forAxis:UILayoutConstraintAxisVertical];
    [_tagLabel setContentHuggingPriority:UILayoutPriorityRequired forAxis:UILayoutConstraintAxisHorizontal];
    [bview addSubview:_tagLabel];
...
- (void)setTagLabelText:(NSString *)tag
{
    UIView *bview = [_tagLabel superview];
    _tagLabel.text = [NSString stringWithFormat:@" %@ ", tag];
    if (tag.length == 0) {
        _tagLabel.hidden = YES;
        [_titleLabel mas_remakeConstraints:^(MASConstraintMaker *make) {
            make.left.mas_equalTo(bview).offset(18);
            make.right.mas_equalTo(-80);
            make.centerY.mas_equalTo(bview);
        }];
    }else{
...
- (void)layoutSubviews
{
    ...
    CGFloat y = self.mainView.sd_height - size.height - 7;
