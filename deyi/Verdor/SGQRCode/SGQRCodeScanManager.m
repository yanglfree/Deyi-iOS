...
- (void)setupSessionPreset:(NSString *)sessionPreset metadataObjectTypes:(NSArray *)metadataObjectTypes currentController:(UIViewController *)currentController {
    ...
    if(authStatus == AVAuthorizationStatusRestricted || authStatus == AVAuthorizationStatusDenied){
        NSString *errorStr = @"应用相机权限未开启,请在设置中启用";
        CGFloat height = [errorStr heightForFont:[UIFont systemFontOfSize:15] width:MAIN_SCREEN_WIDTH-80-30];
        TCAlertView *alert = [[TCAlertView alloc] initWithFrame:[AppDelegate getAppDelegate].window.bounds content:TCAlertViewContentContent|TCAlertViewContentOKBtn|TCAlertViewContentCancelBtn height:120 + height- 16];
        [alert setContent:errorStr];
        [alert setCancelBtnTitle:@"取消"];
        [alert setOKBntTitle:@"去设置"];
        alert.okBtnClicked = ^(UIButton *sender, NSString *input) {
        ...
        alert.cancelBtnClicked = ^(UIButton *sender, NSString *input) {
        };
        [alert showAddParentView:[AppDelegate getAppDelegate].window];
        return;
    }
    ...
    if (!device) {
        [[ToastManager sharedToastManager] showToastType:ToastManagerTypeFailed text:@"未找到相机设备"];
        return;
    }
